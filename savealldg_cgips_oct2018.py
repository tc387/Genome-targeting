...
# ============================================================================================== #
    ...
    strcandStrandList = sorted(list(set(np.floor((np.array(candLocList)+matchslength/2 + (lstrand - gen2strands[0].size))/float(lstrand)))))
    ...
    for iistrand in candStrandList :   # iterate over all candidate strands
        ...
        nurun2.append( Nuwrap(nu_args, nstrands=2, strandlist=[gen1blobs[iiblob].sequence, gen2strands[iistrand].sequence], permutation="1 2"))
        nurun2[iirun-1].run()
        if (nurun2[iirun-1].runstatus == -1):
            raise RuntimeError("Error: NuPack blob-strand " + str(iistrand) + " free energy calc crashed: \n" + nurun2[iirun-1].nu_stdout)
    ...
    sumexpdelG = float(-np.log(np.sum(np.exp(-np.array(deltaGlist))-1.0)+1.0))  # exp delG sum, i.e. partition function
