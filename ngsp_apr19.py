...
def main():
    ...
    for iiblob in range(nblobs):
        print("Finnished.. ", iiblob / float(nblobs))
        # find matching occurrences in genome2
        candLocList = getStrandOverlaps(pattern=str(Seq(str(gen1cblobs[iiblob].sequence)).reverse_complement()),
                                        matchlength=matchslength, string=gen2str)
        candLocList2 = getStrandOverlaps(pattern=str(Seq(str(gen1cblobs[iiblob].sequence)).reverse_complement()),
                                         matchlength=matchslength, string=gen2strc)

        #    print (str(matchslength) + " " + str(Seq(str(gen1blobs[iiblob].sequence)).reverse_complement()) + "\n")
        #    print(candLocList)
        nmatches[iiblob + nblobs] = len(candLocList) + len(candLocList2)
        outfile.write(str(iiblob + nblobs) + " " + str(
            nmatches[iiblob + nblobs].astype(np.float32) / (lblob - matchslength + 1)) + "\n")
        outfile.flush()
