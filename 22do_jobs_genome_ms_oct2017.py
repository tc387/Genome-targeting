...
# make directory and submit job
  ...
  #  subprocess.run(["cp " + "pfunc  cgips.py " + dirname + "/"], shell=True)
    if ( iiblob % blobsperbatch == blobsperbatch-1) : # submit blobsperbatch blobs as a single slurm job
        ...
        dirname0b = "blobcgips_bg-" + gen1file[:-4] + "_sg-"+gen2file[:-4] + "_lb" + str(lblob) + "_ls" + str(lstrand)  + \
       ...
       # batchjobrunargs = r"cd .. ; for ii in $(seq "+str(iblobstart)+" "+str(iblobstart+blobsperbatch-1) + r" ) ; do cd blobcg*_ib$ii ; " + jobrunargs + "$ii > screen.txt ; cd .. ; mv blobcg*_ib$ii " + scratchdirname + "/ ; done" 
        batchjobrunargs = r"cd .. ; for ii in $(seq "+str(iblobstart)+" "+str(iblobstart+blobsperbatch-1) + r" ) ; do cd blobcg*_ib$ii ; " + jobrunargs + "$ii > screen.txt ; cd .. ; done" 
        subprocess.run([r"cat qsubs-master.sh | sed -e 's/JOBNAME11/cGIPS_ib" + str(iblobstart) + "-" + str(iiblob) + r"/' | sed -e 's@JOBRUNARGS11@" + \
...
if (nblobs > iblobstart) : 
    dirname0b = "blobcgips_bg-" + gen1file[:-4] + "_sg-"+gen2file[:-4] + "_lb" + str(lblob) + "_ls" + str(lstrand)  + "_lm" + str(matchslength) + "_ib" + str(iblobstart)
    batchjobrunargs = r"cd .. ;for ii in $(seq "+str(iblobstart)+" "+str(nblobs-1) + r" ) ; do cd blobcg*_ib$ii ; " + jobrunargs + "$ii > screen.txt ; cd .. ; done" 
 
    subprocess.run([r"cat qsubs-master.sh | sed -e 's/JOBNAME11/cGIPS_ib" + str(iblobstart) + "-" + str(nblobs-1) + r"/' | sed -e 's@JOBRUNARGS11@" + \
                    batchjobrunargs + r"@' > " + gdirname+"/"+dirname0b + r"/qsub_local.sh"], shell=True)
    slurmargs=["sbatch qsub_local.sh"]	
    subprocess.Popen(slurmargs,cwd=gdirname+"/"+dirname0b ,shell=True)
