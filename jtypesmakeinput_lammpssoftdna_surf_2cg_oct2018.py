...
def main():
    ...
    ldirname = "surflam-2g-" + gen11file[:-4] + "-" + gen2file[:-4] + "_lb" + str(lblob) + "_ls" + str(lstrand) + \
               "_lm" + str(matchslength) + "_T" + str(Temperature) + "_sd" + str(stranddensity) + "_nst" + str(
        nstrandtypes) + \
    ...
    if load_dGdata:
            ...
            with open(ibfilename, "r") as ibf:
                ...
                for line in ibf:
                    if firstline:
                        ...
                        istrand = int(np.floor(
                            (int(sline[0]) - (basestrandoffset if basestrandoffset != 0 else lstrand)) / float(
                                lstrand)) + 1)  # find the
        ...
        for iblob in range(blobs_per_freepolymer):
            ...
            Qib1 = -np.log(((np.exp(
                -dGt_matrix1[iblob, 0:nstrandtypes]) - 1.0) * ngen2str_counts * stranddensity * rb * rb).sum() + 1.0)
            # print(Qib)
            cbdG_matrix[iblob, 0] = Qib1
            #  print (Qib)
            Qib2 = -np.log(((np.exp(
                -dGt_matrix2[iblob, 0:nstrandtypes]) - 1.0) * ngen2str_counts * stranddensity * rb * rb).sum() + 1.0)
            # print(Qib)
    ...
    rcommand = r"python3 get_input_2cg.py " + str(blobs_per_freepolymer) + " " + str(lencutgenseg) + " " + str(
        lboxxy) + " " + str(lboxz) + " coefficients.dat > data.genome"
...
class Colloid(object):
    ...
    def add_polymer(self, chain_length, sequence):
        polymer = Polymer(self.counters, chain_length, colloid_radius=self.radius, sequence=sequence,
...
class Polymer(object):
    ...
    def build_polymer(self, chain_length, colloid_radius, form):
        ...
        #   print str(vec_random_ndim(3))
            ...
            scale_length = 1 / 1 * math.pow(chain_length,
                                            0.088)  # difference between ideal and self-awoiding walk radius of gyration
            CM = np.sum(self.coordinates[:, :], axis=0) / chain_length  # centre of mass
